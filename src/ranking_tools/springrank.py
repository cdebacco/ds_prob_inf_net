...
class SpringRank:
    ...
    def fit(self, A):
        ...
        self.depth = self.ranks.max() - self.ranks.min()
        self.n_levels = self.get_n_levels()
    ...
    def get_rescaled_ranks(self, target_scale, delta_s: float = 1):
        """Rescale ranks using target scale and inverse temperature"""
        if self.is_fitted_beta_ == False:
            self.beta = self._get_inverse_temperature()
            self.is_fitted_beta_ = True
        scaling_factor = delta_s / (np.log(target_scale / (1 - target_scale)) /
                            (2 * self.beta))
    ...
    def get_n_levels(self, q: float = 0.75, method: str = 'min-max'):
        '''
        Get number of levels such that each level represents a reference probability of winning q
        '''
        if self.is_fitted_beta_ == False:
            self.beta = self._get_inverse_temperature()
            self.is_fitted_beta_ = True
        if method == 'min-max':
            delta_hat = self.depth
            self.delta_beta =  np.log(q/(1-q)) / (2 * self.beta)
            return delta_hat / self.delta_beta
        else:
            raise NotImplementedError(f"method={method} not yet implemented!")


def calculate_n_levels(delta_hat: float, beta: float, q: float = 0.75)-> float:
    delta_beta = np.log(q / (1 - q)) / (2 * beta)
    return delta_hat / delta_beta
